...
void BST::initialize_grid(const Grid& G, Traj lyap, Measurement m){
    ...
    auto* dead_node = new TreeNode(-1, blank_c); dead_node->cell.i_nodes = {dead_node, dead_node, dead_node, dead_node, dead_node, dead_node}; dead_node->cell.k_nodes = {dead_node, dead_node, dead_node, dead_node, dead_node, dead_node};
    dead = dead_node;
        ...
        for (int j = int(round((-3*m.std[1])/G.del[1])); j <= int(round((3*m.std[1])/G.del[1])); j++){
            for (int k = int(round((-3*m.std[2])/G.del[2])); k <= int(round((3*m.std[2])/G.del[2])); k++){
                for (int l = int(round((-3*m.std[3])/G.del[3])); l <= int(round((3*m.std[3])/G.del[3])); l++){
                    for (int n = int(round((-3*m.std[4])/G.del[4])); n <= int(round((3*m.std[4])/G.del[4])); n++){
                        for (int p = int(round((-3*m.std[5])/G.del[5])); p <= int(round((3*m.std[5])/G.del[5])); p++){

                            current_state = {i,j,k,l,n,p}; key = state_conversion(current_state.data());
                            arma::vec current_state_vec = {i*G.del[0], j*G.del[1], k*G.del[2], l*G.del[3], n*G.del[4], p*G.del[5]};
...
void BST::initialize_vuw(const Grid& G, Traj lyap, TreeNode* r){
    ...
    if(r->cell.new_f==0){
        ...
        double v6 = 0;
        if(G.oetype==0){
            v6 = pow(lyap.mu / pow(x[0], 3), 0.5); // Mean anomaly
        }else if(G.oetype==1){
            v6 = pow(lyap.mu*x[0],0.5)*pow((1+x[1]*cos(x[5]+G.del[5]/2)+x[2]*sin(x[5]+G.del[5]/2))/x[0],2.0); // True longitude
        }
