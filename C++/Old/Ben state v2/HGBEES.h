...
CLASS DEFINITIONS
==============================================================================*/
struct Cell{  // All of the variables held at a particular cell
    double prob;
    vector<double> v; 
    vector<double> u; 
    vector<double> w; 
...
struct VectorHasher {    // Vector Hashing Function for Hash Table
...
class HGBEES{       // HGBEES Class
  public:
    ...
    vector<vector<int>> inactive_keys;
    vector<vector<int>> small_keys;
    vector<vector<int>> big_keys;
    int n;        
...
void HGBEES::Initialize_vuw(Grid G,Lorenz3D Lor){
    vector<int> current_key; vector<double> x(G.d,0); get_keys(G);
    
    for (int l = 0; l < inactive_keys.size(); l++){
        current_key = inactive_keys[l]; 
...
void HGBEES::Modify_pointset(Grid G,Lorenz3D Lor){
    ...
    for(int l = 0; l < big_keys.size(); l++){   // Check/Create Neighbors of Big Cells
        prob_sum += P[keys[l]].prob;
        vector<int> current_key = big_keys[l];
    ...
    for (int l = 0; l < small_keys.size(); l++){     // Remove small cells that do not neighbor big cells in the upwind direction
        current_key = small_keys[l];
    ...
    for (int i = 0; i < n; i++){        // Normalizing Probability
        P[keys[i]].prob = max(P[keys[i]].prob,0.)/prob_sum; 
    }
};
...
vector<double> HGBEES::RHS_P(Grid G,Lorenz3D Lor){
    ...
    for(int q = 0; q < G.d; q++){  // Compute Corner Transport Upwind (CTU) flux terms
        for(int l = 0; l < n; l++){ 
            vector<int> l_key = keys[l]; 
            if(l_key != DEAD){
                ...
                if ((P[l_key].prob>G.thresh)||(P[i_key].prob>G.thresh)){
                    ...
                    double th,t; 
                    if (P[i_key].v[q]>0){   // Compute Second-order Correction Flux Term
                        th = (P[i_key].prob-P[i_i_key].prob)/(P[l_key].prob-P[i_key].prob);
                    }else{
                        th = (P[k_key].prob-P[l_key].prob)/(P[l_key].prob-P[i_key].prob);
                    }

                    t = abs(P[i_key].v[q]); P[i_key].f[q] += t*(G.del/G.dt - t)*F*MC(th);                                      
                }
            }
        }
    }
    for(int l = 0; l < n; l++){  // Calculating K
        vector<int> l_key = keys[l]; 
        if(l_key != DEAD){
            for(int q = 0; q < G.d; q++){
                vector<int> i_key = l_key; i_key[q] = i_key[q] - 1; if(P.count(i_key)==0) i_key = DEAD;  
...
void HGBEES::get_keys(Grid G){
    n = P.size(); keys.clear(); inactive_keys.clear(); small_keys.clear(); big_keys.clear();
    ...
    for (auto it = P.begin(); it != P.end(); it++) {
        ...
        if (current_key != DEAD){
            ...
            if(P[current_key].active == 0){
                inactive_keys.push_back(current_key);
            }else{
                if(P[current_key].prob >= G.thresh){
                    big_keys.push_back(current_key);
                }else{
                    small_keys.push_back(current_key);
                }
            }
        } 
    }
...
void HGBEES::Record_Data(string file_name, Grid G){
	ofstream myfile; myfile.open(file_name); n = P.size();
    for(int i = 0; i < n; i++){
