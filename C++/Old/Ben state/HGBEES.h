...
void HGBEES::Initialize_vuw(Grid G,Lorenz3D Lor){
    vector<int> current_key; vector<double> x(G.d,0); get_keys();
...
void HGBEES::Modify_pointset(Grid G,Lorenz3D Lor){
    double prob_sum = 0; vector<vector<int>> dead_keys; 
    ...
    for (int l = 0; l < dead_keys.size(); l++){     // Remove small cells that do not neighbor big cells in the upwind direction
        vector<int> current_key = dead_keys[l];
        if(current_key != DEAD){
...
void HGBEES::get_keys(){
    n = P.size(); vector<vector<int>> all_keys(n, vector<int>(3,0)); keys = all_keys;
