...
class HGBEES{       // HGBEES Class
  public:
    ...
    unordered_map<int, vector<double>> w;
    unordered_map<int, vector<double>> f;
    vector<int> keys;
...
vector<double> HGBEES::RHS_P(Grid G,Lorenz3D Lor){
    vector<double> K(n,0.0); n = P.size(); get_keys_values(); vector<double> zeros(G.d,0);
