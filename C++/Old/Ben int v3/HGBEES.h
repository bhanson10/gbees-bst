...
void HGBEES::Initialize_vuw(Grid G,Lorenz3D Lor){
    int current_key; vector<int> current_state; vector<double> x(G.d,0); 
...
void HGBEES::Modify_pointset(Grid G,Lorenz3D Lor){
    double prob_sum = 0; vector<int> dead_keys; int current_key; 

    for(auto& it: P){   // Check/Create Neighbors of Big Cells
        current_key = it.first; 
        if (P[current_key].prob >= G.thresh){
            ...
            dead_keys.push_back(current_key);
        }
    }   
...
void HGBEES::RHS_P(Grid G,Lorenz3D Lor){
    ...
    for(auto& it: P){ //Calculating Initial f
        int l_key = it.first; P[l_key].K = 0.0;
        if(l_key != -1){
    ...
    for(int q = 0; q < G.d; q++){
        for(auto& it: P){ // Calculating Total f
            int l_key = it.first; 
            if(l_key != -1){
                vector<int> l_state = P[l_key].state; 
