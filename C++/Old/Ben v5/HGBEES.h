...
bool HGBEES::no_neighbors(Grid G, Lorenz3D Lor, vector<int> current_key){
    ...
    for (int i = current_key[1]-1; i <= current_key[1]+1; i++){
        for (int j = current_key[0]-1; j <= current_key[0]+1; j++){
            for (int k = current_key[2]-1; k <= current_key[2]+1; k++){
                vector<int> new_key = {j, i, k};
