...
class GBEES{       // HGBEES Class
  public:
    ...
    void normalize_prob(TreeNode* r, double prob_sum, Grid G);
...
void GBEES::Modify_pointset(Grid G, Lorenz3D Lor){
    ...
    Initialize_ik_nodes(G,P.root); 
    Initialize_vuw(G, Lor, P.root); 
    normalize_prob(P.root, prob_sum, G);
...
void GBEES::normalize_prob(TreeNode* r, double prob_sum, Grid G){
    if (r==NULL){
        return;
    }
    normalize_prob(r->left, prob_sum, G);
    normalize_prob(r->right, prob_sum, G);

    double current_p = r->cell.prob;

    r->cell.prob = current_p/(G.del[0]*G.del[1]*G.del[2]*prob_sum); 
};
