...
int main() {
    ...
    std::cout << "Timestep: 0, Program time: 0 s, Sim. time: 0 s, Active/Total Cells: " << D.P.active_size(D.P.root, G) << "/" << D.P.full_size(D.P.root) << std::endl;
    ...
    for (int i = 1; i <= num_step; i++){
        ...
        if (i % record_step == 0){ 
            ...
            std::cout << "Timestep: " << i << ", Program time: " << elapsed.count() << " s, Sim. time: " << i*G.dt << " s, Active/Total Cells: " << D.P.active_size(D.P.root, G) << "/" << D.P.full_size(D.P.root) << std::endl;
