...
void GBEES::initial_f(Grid G, TreeNode* r){
    ...
    if(l_key != -1){
        ...
        for(int q = 0; q < DIM; q++){
            ...
            TreeNode* k_node = P.recursiveSearch(P.root, k_key); if(k_node == NULL) k_node = dead; 
...
void GBEES::total_f(Grid G, TreeNode* r){
    ...
    if(l_key != -1){
        ...
        for(int q = 0; q < DIM; q++){
            ...
            TreeNode* i_node = P.recursiveSearch(P.root, i_key); if(i_node==NULL) i_node = dead; 
            if ((r->cell.prob >= G.thresh)||(i_node->cell.prob >= G.thresh)){
                ...
                if (i_node->cell.v[q]>0){
                    ...
                    th = (k_node->cell.prob-r->cell.prob)/(r->cell.prob-i_node->cell.prob);
                }
...
void GBEES::calculating_K(Grid G, TreeNode* r){
    ...
    if(l_key != -1){
        ...
        for(int q = 0; q < DIM; q++){
            std::array<int,DIM> i_state = l_state; i_state[q] = i_state[q]-1; int i_key = state_conversion(i_state);
            TreeNode* i_node = P.recursiveSearch(P.root, i_key); if(i_node==NULL) i_node = dead; 
            r->cell.K -= (r->cell.f[q]-i_node->cell.f[q])/G.del;  
        }
    }
};
