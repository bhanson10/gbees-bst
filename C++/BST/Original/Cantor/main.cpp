...
int main() {
    ...
    for(int nm = 0; nm < num_meas; nm++){
        ...
        D.Modify_pointset(G,Lor); D.Record_Data(filename, G, D.P.root); 
        std::cout << "Timestep: " << nm << "-0, Program time: " << elapsed.count() << " s, Sim. time: " << sim_time << " s, Active/Total Cells: " << D.P.active_size(D.P.root, G) << "/" << D.P.full_size(D.P.root) << std::endl;
