...
void HGBEES::RHS_P(Grid G,Lorenz3D Lor){
    ...
    for(auto it = P.begin(); it != P.end(); it++){ //Calculating Initial f
        ...
        if(l_key != -1){
            ...
            for(int q = 0; q < G.d; q++){
                memcpy(k_state, l_state, sizeof(l_state)); k_state[q] = k_state[q]+1; int k_key = state_conversion(k_state,G);
        ...
        for(auto it = P.begin(); it != P.end(); it++){ // Calculating Total f
            ...
            if(l_key != -1){
                ...
                if ((P[l_key].prob>G.thresh)||(P[i_key].prob>G.thresh)){
                    ...
                    for(int e = 0; e < G.d; e++){
                        if (e!=q){
                            memcpy(j_state, l_state, sizeof(l_state)); j_state[e]=j_state[e]-1; 
                            int j_key = state_conversion(j_state,G); if(P.count(j_key)==0) j_key = -1; 
                            memcpy(p_state, i_state, sizeof(i_state)); p_state[e] = p_state[e]-1; 
                    ...
                    memcpy(i_i_state, i_state, sizeof(i_state)); i_i_state[q] = i_i_state[q]-1;
                    int i_i_key = state_conversion(i_i_state,G); if(P.count(i_i_key)==0) i_i_key = -1; 
                    memcpy(k_state, l_state, sizeof(l_state)); k_state[q] = k_state[q] + 1; 
    ...
    for (int i = current_state[1]-1; i <= current_state[1]+1; i++){
        for (int j = current_state[0]-1; j <= current_state[0]+1; j++){
            for (int k = current_state[2]-1; k <= current_state[2]+1; k++){
                int new_state[] = {j, i, k}; int new_key = state_conversion(new_state,G);
