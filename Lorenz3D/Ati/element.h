...
inline bool operator<(const key& a, const key& b) {
	int c=2;
	for (int n=0;n<DIMS;n++) {
		(a.ijk[n]<b.ijk[n]) ? c=1 : 0;
		(a.ijk[n]>b.ijk[n]) ? c=0 : 0;
	}
	(a.dummy > b.dummy) ? c=1 : 0;
	(a.dummy < b.dummy) ? c=0 : 0;
	(c==2) ? c=0 : 0;
	return c;
}
...
public:
	...
	// Ben attempting to fix Ati's code
	element& operator=(const element&);	//	assignment maintains iterators
};
