...
int main(){
    ...
    while(tt < Lor.T){ // Time of uncertainty propagation
        ...
        std::string filename = "./MC Freq Data/M" + std::to_string(nm) + "/MC_0.txt"; M.Record_Data(filename, 0); 
        ...
        while(mt < measure_time){ // Time between measurements 
            ...
            if(RECORD){ // Record MC 
                ...
                //std::cout << std::endl; 
                std::string filename = "./MC Freq Data/M" + std::to_string(nm) + "/MC_" + std::to_string(record_count) + ".txt"; M.Record_Data(filename, mt + rt);
                record_count+=1;
            }
