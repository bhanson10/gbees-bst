...
void GridBayes::initialize() {
  ...
  */
  ...
  while (kk[0] != ki[0][2]) {
    ...
    list.pdf[n_list] = exp(-temp_d/2);
    //list.pdf[n_list] = exp(-2*temp_d) / pdf_den;
  ...
  mm = -1;  // Latest nontrivial PDF index
...
void GridBayes::record_data(string file_name) {
  ...
  // list.nb shows the list.mm instead.
	...
	for (int i = 0; i < mm; i++) {
		...
		// PDF
    if(list.pdf[i] >= PDF_THRESH){
      myfile << list.pdf[i];   // pdf
      ...
      for (int j = 0; j < N_DIM; j++) {
          ...
          else 			    myfile << " " << list.pos[i][j]*0.5;  // state value
      }
