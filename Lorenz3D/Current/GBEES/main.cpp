...
int main(){
    ...
    std::ifstream measurement_file("./Data/measurements.txt"); 
    ...
    D.Initialize_D(G,Lor); D.normalize_tree(G, D.P.root);
    ...
    double tt = 0; int nm = 0; int record_count; double rt; double mt; int step_count; std::ofstream time_file; 
    while(tt < Lor.T){ // Time of uncertainty propagation
        ...
        std::cout << " TU, Active/Total Cells: " << D.a_count << "/" << D.tot_count << std::endl; 
        ...
        while(mt < measure_time){ // Time between measurements 
            ...
            while(rt < record_time){ // Time between recording the PDF
                ...
                D.Modify_pointset(G,Lor); D.check_CFL_condition(G, D.P.root, D.cfl_min_dt); G.dt = std::min(D.cfl_min_dt, record_time-rt); rt += G.dt;  
                //D.Modify_pointset(G,Lor); D.check_CFL_condition(G, D.P.root, D.cfl_min_dt); G.dt = std::min(1E-5, record_time-rt); rt += G.dt;  
                D.RHS(G, Lor); D.update_prob(G, D.P.root); D.normalize_tree(G, D.P.root);  
        ...
        if((MEASURE)&&(tt < Lor.T)){ // Perform discrete measurement update
            ...
            Measurement m; 
            m.mean = {measurements[nm][0], measurements[nm][1], measurements[nm][2]}; m.std = G.std; 
