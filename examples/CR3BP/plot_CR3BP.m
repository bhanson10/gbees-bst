...
const.U = [const.LU, const.LU, const.LU, const.LU/const.TU, const.LU/const.TU, const.LU/const.TU]; 
const.T = 2.6513344042156235E+0; const.r = 1560.8;
rv.start = [1.017714765; -1.069793E-20; 3.408583E-34; -1.197784E-13; 1.187104E-2; -3.959579E-35]; 
ROWS = 2; COLS = 3; 
...
lbl.XString = '$v_x$ (km/s)'; lbl.YString = '$v_y$ (km/s)'; lbl.YString = '$v_z$ (km/s)';
initialize_figures('n', 2, 'spacing', {[800 100 700 700]}, 'lbl', lbl, 'axs', {'equal'}); 
set(gca, 'FontName', 'Times', 'FontSize', 14);

initialize_figures('n', 3, 'spacing', {[50 100 700 700]}, 'axs', {'equal'}); 
set(gca, 'FontName', 'Times', 'FontSize', 14);
tiledlayout(ROWS,COLS,"TileSpacing","compact");

initialize_figures('n', 4, 'spacing', {[800 100 700 700]}, 'axs', {'equal'}); 
set(gca, 'FontName', 'Times', 'FontSize', 14); 
tiledlayout(ROWS,COLS,"TileSpacing","compact");
...
[t, x] = ode87(@(t, x) CR3BP(t, x, const), tspan, x0, options);
...
figure(1); 
plot3(x((t <= 14*3600),1),x((t <= 14*3600),2),x((t <= 14*3600),3),'r-','LineWidth',2,'DisplayName','Nominal');
plot3(x(:,1),x(:,2),x(:,3),'r--','LineWidth',1,'DisplayName','Nominal');
drawnow;
figure(2); 
plot3(x((t <= 14*3600),4),x((t <= 14*3600),5),x((t <= 14*3600),6),'r-','LineWidth',2,'DisplayName','Nominal');
...
NM = 1; 
alphavalue = 0.1;
P_DIR = "./results/PCR3BP/pf/PDF0/P"; 
...
for nm=0:NM-1
    ...
    for i=[0, num_files - 1]
        ...
        [x_pf, P_pf, n_pf, t_pf(count)] = parse_nongaussian_txt(P_FILE, const);
        ...
        figure(1); 
        scatter(x_pf(:,1), x_pf(:,2), 10, 'filled', 'k', ...
            'MarkerEdgeAlpha', alphavalue, 'MarkerFaceAlpha', alphavalue);
        figure(3); 
        nexttile(1 + (count-1)*COLS); hold all; 
        scatter(x_pf(:,1), x_pf(:,2), 10, 'filled', 'k', ...
            'MarkerEdgeAlpha', alphavalue, 'MarkerFaceAlpha', alphavalue);
        figure(2); 
        scatter(x_pf(:,3), x_pf(:,4), 10, 'filled', 'k', ...
            'MarkerEdgeAlpha', alphavalue, 'MarkerFaceAlpha', alphavalue);
        figure(4); 
        nexttile(1 + (count-1)*COLS); hold all; 
        scatter(x_pf(:,3), x_pf(:,4), 10, 'filled', 'k', ...
...
%% GBEES
NM = 1; 
clear p; p.name = "GBEES"; p.alpha = [0.1, 0.2, 0.6]; p.color = 'b';
P_DIR = "./results/CR3BP/gbees/PDF0/P"; 
...
for nm=0:NM-1
    ...
    for i=[0, num_files - 1]
        ...
        drawnow; 
        % figure(3); 
        % nexttile(2 + (count-1)*COLS); hold all;
        % plot_nongaussian_surface(x_gbees(:,1:3),P_gbees,[normpdf(1)/normpdf(0), normpdf(2)/normpdf(0), normpdf(3)/normpdf(0)],p);
        figure(2); 
...
%                                FUNCTIONS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function x1 = CR3BP(t, x, const)
    x1 = [x(4); x(5); x(6); 2*x(5)+x(1)-(const.mu*(x(1)-1+const.mu)/(((x(1)-1+const.mu)^2+x(2)^2+x(3)^2)^(1.5)))-((1-const.mu)*(x(1)+const.mu)/(((x(1)+const.mu)^2+x(2)^2+x(3)^2)^(1.5))); -2*x(4)+x(2)-(const.mu*x(2)/(((x(1)-1+const.mu)^2+x(2)^2+x(3)^2)^(1.5)))-((1-const.mu)*x(2)/(((x(1)+const.mu)^2+x(2)^2+x(3)^2)^(1.5))); -(const.mu*x(3)/(((x(1)-1+const.mu)^2+x(2)^2+x(3)^2)^(1.5)))-((1-const.mu)*x(3)/(((x(1)+const.mu)^2+x(2)^2+x(3)^2)^(1.5)))];
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [x, P, n, t] = parse_nongaussian_txt(filename, const)
    ...
    while ~feof(fileID)
        ...
        x(count, :) = [str2double(line{2});str2double(line{3});str2double(line{4});str2double(line{5});str2double(line{6});str2double(line{7})].*const.U';
        count = count + 1; 
    end
