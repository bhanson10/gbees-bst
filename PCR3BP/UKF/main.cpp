...
vec PCR3BP(double t, vec y, double mu){
    double r1 = pow(pow(y(0)+mu,2.0)+pow(y(1),2.0),1.5);
    double r2 = pow(pow(y(0)-1+mu,2.0)+pow(y(1),2.0),1.5);
    double ax = 2*y(3)+y(0)-(mu*(y(0)-1+mu)/r2)-((1-mu)*(y(0)+mu)/r1);
    double ay = -2*y(2)+y(1)-(mu*y(1)/r2)-((1-mu)*(y(1)+mu)/r1);
...
vec RK87(double t0, double t1, vec y, double h, Traj LPO){
    double tol = 1e-6;
    double hmax = 1e-3;
    int n_reject = 0;
    int reject = 0;
    ...
    while (t < tfinal) {
        ...
        f.col(0) = PCR3BP(t, y, LPO.mu);
        for (int j = 0; j <= 11; j++) {
            vec yh = y + h * f * a.col(j);
            f.col(j + 1) = PCR3BP(t + c1(j) * h, yh, LPO.mu);
        }
...
int main(){
    ...
    bool RECORD = true; // Write UKF to .txt file (for epochs)
    ...
    if (body == 0) {
        FILE_PATH = "/Users/bhanson/Library/CloudStorage/OneDrive-UCSanDiego/UCSD/Research/GBEES/GBEES/PCR3BP/UKF/cmake-build-debug/Epochs/Europa";
    }
    //===========================================================================================================
    ...
    Traj LPO{}; // Trajectory object
    std::getline(measurement_file, line); // Skip label line
    std::getline(measurement_file, line); LPO.mu = std::stod(line); // Read in mu
    std::getline(measurement_file, line); // Skip label line
    std::getline(measurement_file, line); LPO.T = std::stod(line);  // Read in T

    vec tspan = linspace<vec>(0, LPO.T, num_dist+1);

    measurement_file.close();
    //===========================================================================================================
    const double alpha = 1e-3; // Alpha, spread of the sigma points around mean
    const double beta  = 2.0;  // Beta, incorporates prior knowledge
    const double kappa = 0.0;  // Kappa, secondary scaling parameter
    double lam         = pow(alpha, 2.0)*(DIM+kappa)-DIM; // Lambda
    vec Wm(2*DIM+1,fill::zeros); // Weights, m
    vec Wc(2*DIM+1,fill::zeros); // Weights, c

    // Defining Weights/Sigma Points
    Wm(0) = lam/(DIM + lam);
    Wc(0) = (lam/(DIM + lam)) + (1-pow(alpha,2.0)+beta);
    ...
    std::cout << "Program time: " << elapsed.count() << " s, Sim. time: " << 0 << " TU" << std::endl;
    if(RECORD){std::string filename = FILE_PATH + "/M" + std::to_string(mn) + "/pdf_0.txt"; Record_Data(filename, mean, cov, 0);}

    double h = 1e-3; int record_count = 1;
    for(int i=1; i < tspan.n_elem; i++){
        // Calculate Sigma Points
        mat A(DIM,2*DIM+1,fill::zeros);
        mat L = real(sqrtmat((DIM+lam)*cov));
        A.col(0) = mean;
        for(int j=1; j < DIM+1; j++){
            A.col(j) = mean + L.row(j-1).t();
        }
        for(int j=DIM+1; j < 2*DIM+1; j++){
            A.col(j) = mean - L.row(j-DIM-1).t();
        }

        // Prediction
        for(int j=0; j < 2*DIM+1; j++) {
            A.col(j) = RK87(tspan(i-1), tspan(i),A.col(j), h, LPO);
        }
        ...
        if(OUTPUT){
            ...
            std::cout << "Program time: " << elapsed.count() << " s, Sim. time: " << tspan(i) << " TU" << std::endl;
        }

        if(RECORD){ // Record PF Scatter Plot
            std::string filename = FILE_PATH + "/M" + std::to_string(mn) + "/pdf_" + std::to_string(record_count) + ".txt"; Record_Data(filename, mean, cov, tspan(i));
            record_count+=1;
        }
    }
