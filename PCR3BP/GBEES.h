    ...
    for (int i = round((-2*G.std[0])/G.del[0]); i <= round((2*G.std[0])/G.del[0]); i++){
        for (int j = round((-2*G.std[1])/G.del[1]); j <= round((2*G.std[1])/G.del[1]); j++){
            for (int k = round((-2*G.std[2])/G.del[2]); k <= round((2*G.std[2])/G.del[2]); k++){
                for (int l = round((-2*G.std[2])/G.del[2]); l <= round((2*G.std[2])/G.del[2]); l++){
                ...
                for (int j = n_states[1][0]; j <= n_states[1][1]; j++){
                    for (int k = n_states[2][0]; k <= n_states[2][1]; k++){
                        for (int l = n_states[3][0]; l <= n_states[3][1]; l++){
                            std::array<int,DIM> state_update = {i, j, k, l}; 
                            
                            TreeNode* neighbor_node = r;
                            for (int m = 0; m < DIM; m++){
                                if(state_update[m] == 1){
                                    neighbor_node = neighbor_node->cell.k_nodes[m];
                                }else if (state_update[m] == -1){
                                    neighbor_node = neighbor_node->cell.i_nodes[m];
                                }

                                if(neighbor_node == dead){
                                    std::array<int,DIM> new_state = current_state; 
                                    for(int n = 0; n < DIM; n++){
                                        new_state[n] += state_update[n]; 
                                    }
            ...
            for (int j = 0; j < num_n[1]; j++){
                for (int k = 0; k < num_n[2]; k++){
                    for (int l = 0; l < num_n[3]; l++){
                        std::array<int,DIM> new_state = {which_n[0][i], which_n[1][j], which_n[2][k], which_n[3][l]}; uint64_t new_key = state_conversion(new_state,G);
