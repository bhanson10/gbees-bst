...
class BST{ // Binary Search Tree - data structure storing information
    public: 
        ...
        void writeFile2(std::ofstream& myfile, Grid G, TreeNode* r);
        void writeFile3(std::ofstream& myfile, Grid G, TreeNode* r);
        void writeFile4(std::ofstream& myfile, Grid G, TreeNode* r);
};
...
void BST::writeFile2(std::ofstream& myfile, Grid G, TreeNode* r){
    if (r == NULL){
        return; 
    }

    writeFile2(myfile, G, r->left);
    writeFile2(myfile, G, r->right);
    ...
    if (r->cell.prob > G.thresh){
        ...
        myfile << ", VY Forward node: ";
        for(int i = 0; i < DIM; i++){
            myfile << " " << r->cell.k_nodes[3]->cell.v[i];
        }
        myfile << std::endl; 
    }
};

void BST::writeFile3(std::ofstream& myfile, Grid G, TreeNode* r){
    if (r == NULL){
        return; 
    }

    writeFile3(myfile, G, r->left);
    writeFile3(myfile, G, r->right);
    ...
    if (r->cell.prob > G.thresh){
        ...
        myfile << ", Advection: ";
        for(int i = 0; i < DIM; i++){
            myfile << " " << std::setprecision(10) << r->cell.v[i];
        }
        myfile << std::endl; 
    }
};

void BST::writeFile4(std::ofstream& myfile, Grid G, TreeNode* r){
    if (r == NULL){
        return; 
    }

    writeFile4(myfile, G, r->left);
    writeFile4(myfile, G, r->right);
