...
public:
    ...
    void Record_Data(const std::string& filename, double t){
        ...
        for (Particle& p : particles){
            myfile << p.P << " " << p.state(0) << " " << p.state(1) << " " << p.state(2) << " " << p.state(3) << std::endl;
        }
    }

    void RK87(double t0, double t1, double h, Traj LPO){
...
int main(){
    ...
    if (body == 0) {
        FILE_PATH = "/Users/bhanson/Library/CloudStorage/OneDrive-UCSanDiego/UCSD/Research/GBEES/GBEES/PCR3BP/PF/cmake-build-debug/Epochs/Europa";
    }
    //===========================================================================================================
    ...
    std::cout << "Program time: " << elapsed.count() << " s, Sim. time: " << 0 << " TU, Particles: " << M.size << std::endl;
    if(RECORD){std::string filename = FILE_PATH + "/M" + std::to_string(mn) + "/pdf_0.txt"; M.Record_Data(filename, 0);};

    double h = 1e-3; int record_count = 1;
    for(int i=1; i < tspan.n_elem; i++){
        M.RK87(tspan(i-1), tspan(i), h, LPO);

        if(OUTPUT){
            finish = std::chrono::high_resolution_clock::now(); elapsed = finish - start;
            std::cout << "Program time: " << elapsed.count() << " s, Sim. time: " << tspan(i) << " TU, Particles: " << M.size << std::endl;
        }

        if(RECORD){ // Record PF Scatter Plot
            std::string filename = FILE_PATH + "/M" + std::to_string(mn) + "/pdf_" + std::to_string(record_count) + ".txt"; M.Record_Data(filename, tspan(i));
            record_count+=1;
        }
    }
