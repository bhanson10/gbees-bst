...
TreeNode* initialize_grid(Grid* G, Meas M, Traj T){
    TreeNode* P = NULL; 
...
void grow_tree(TreeNode** P, Grid G, Traj T){
...
void update_prob_recursive(TreeNode* r, Grid* G){
    ...
    for(int i = 0; i < G->dim; i++){
        TreeNode* i_node = r->i_nodes[i]; 
        if(i_node != NULL){
            r->prob -= (G->dt/G->dx[i])*(r->ctu[i]-r->i_nodes[i]->ctu[i]);
        }else{
...
void update_prob(TreeNode* P, Grid* G, double rt){
    G->dt = DBL_MAX; 
    check_cfl_condition(P, G); 
    G->dt = fmin(G->dt, rt);
    godunov_method(&P, *G); 
    update_prob_recursive(P, G);
    normalize_tree(P); 
}
