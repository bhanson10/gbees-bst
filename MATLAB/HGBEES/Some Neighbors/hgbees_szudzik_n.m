...
function state = key_conversion(key,G)
    shift_state = SzudzikUnpair(key,G.d);
...
function [K]=RHS_P(D,G)
    ...
    D.f(D.keys) = {zeros(1,G.d)}; states = cell(D.n-1);
    for l=2:D.n
        l_key = D.keys(l); l_state = key_conversion(l_key,G); states{l-1} = l_state;
