function hgbees_cantor_n
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% begin user input %%%%%%%%%%%%%%%%%%%%%%%%%%%% 
T=1; G.thresh=0.00002; G.start=[-11.5; -10; 9.5]; G.dx=0.4;
G.dt=.0005; dt=.005; G.xh=G.dx/2; G.d=3; G.sigma=4; G.b=1; G.r=48; G.L=30;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% end of user input %%%%%%%%%%%%%%%%%%%%%%%%%%% 
[hD] = Initialize_D(G); 
...
for timestep=1:5, disp("Timestep: " + string(timestep)); t=t+G.dt; if mod(timestep,1)==0, [hD]=Modify_pointset(hD,G); end
...
function [D] = Initialize_D(G)
    D.P = dictionary(); D.P(-1) = 0;
    D.v = dictionary(); D.v(-1) = {zeros(1,G.d)};
...
function [D] = Modify_pointset(D,G) 
    ...
    for l=2:D.m                       % Check/Create Neighbors of Big Cells
        if(D.P(D.keys(l))>=G.thresh)
            ...
            v = D.v(current_key); v = v{1}; 
            
            if(v(1)>0), x_neighbors = state(1):state(1)+1; elseif(v(1)==0), x_neighbors = state(1)-1:state(1)+1; else, x_neighbors = state(1)-1:state(1); end
            if(v(2)>0), y_neighbors = state(2):state(2)+1; elseif(v(2)==0), y_neighbors = state(2)-1:state(2)+1; else, y_neighbors = state(2)-1:state(2); end
            if(v(3)>0), z_neighbors = state(3):state(3)+1; elseif(v(3)==0), z_neighbors = state(3)-1:state(3)+1; else, z_neighbors = state(3)-1:state(3); end

            for i=y_neighbors(1):y_neighbors(end)
                for j=x_neighbors(1):x_neighbors(end)
                    for k=z_neighbors(1):z_neighbors(end)
                        new_state = [j i k]; new_key = state_conversion(new_state, G);
                        if(~isKey(D.P, new_key)), D.P(new_key) = 0; count = count + 1; end
                    end
                end
            end  
        end
    end
    disp("Added: " + string(count));
...
function [K]=RHS_P(D,G)
    ...
    for l=2:D.n
        ...
        f_l = zeros(G.d); u_l = D.u(l_key); u_l = u_l{1}; w_l = D.w(l_key); w_l = w_l{1};  
        for d=1:G.d
            k_state = l_state; k_state(d) = k_state(d)+1; k_key = state_conversion(k_state,G);
            if(~isKey(D.P,k_key)),k_key = -1;end
    ...
    for d=1:G.d
        for l=2:D.n
            ...
            if(~isKey(D.P,i_key)),i_key = -1;end
            ...
            if (D.P(l_key)>G.thresh)||(D.P(i_key)>G.thresh)
                ...
                for e=1:G.d
                    if e~=d
                        ...
                        if(~isKey(D.P,j_key)),j_key = -1;end
                        ...
                        if(~isKey(D.P,p_key)),p_key = -1;end
                ...
                if(~isKey(D.P,i_i_key)),i_i_key = -1;end
                k_state = l_state; k_state(d) = k_state(d)+1; k_key = state_conversion(k_state,G);
                if(~isKey(D.P,k_key)),k_key = -1;end
                if(v_i(d)>0)
