...
int main(){
    std::ifstream measurement_file("measurements_SE.txt");
    std::ifstream size_file("size_SE.txt"); 
    ...
    //===================================== Begin User Input ======================================
    MC M; M.std  = {1E-4, 1E-4, 5E-2, 5E-2}; M.size = size[0]; 
    M.epoch = {measurements[0][0], measurements[0][1], measurements[0][2], measurements[0][3]};
    Traj lyap; lyap.mu = 3.054200000000000E-06; lyap.T = 0.03141584257;
    double measure_time = lyap.T; double record_time = measure_time/5; bool RECORD = true; bool RECORD_F = true; bool MEASURE = true;
    ...
    while(tt < lyap.T){ // Time of uncertainty propagation
        ...
        while(mt < measure_time){ // Time between measurements 
            ...
            while(rt < record_time){ // Time between recording the PDF
                std::string t; getline(time_file, t); 
                double dt = std::stod(t)-init_time; init_time = std::stod(t); rt += dt; 
