...
int main(){
    ...
    std::ifstream measurement_file("./Data/measurements_JE.txt"); 
    ...
    size_file.open("./MC/size_JE.txt");
    //===================================== Begin User Input ======================================
    Grid G; G.thresh = 3E-7; G.epoch = {measurements[0][0], measurements[0][1], measurements[0][2], measurements[0][3]};
    G.std = {1.5E-3, 1.5E-3, 7.5E-3, 7.5E-3}; G.del = {1.5E-3, 1.5E-3, 7.5E-3, 7.5E-3}; 
    G.pair = 2; /*1: Cantor, 2: Rosenberg*/ G.del_method = 1; /*1. Steps, 2. %*/
    Traj lyap; lyap.mu = 2.528017528540000E-5; lyap.T = 6.2830621392033006;
    double measure_time = lyap.T/3; double record_time = measure_time/3; bool RECORD = true; bool MEASURE = true;
    ...
    while(tt < lyap.T){ // Time of uncertainty propagation
        ...
        time_file.open("./MC/Times/time" + std::to_string(nm) + ".txt"); time_file << 0 << std::endl; 
